...
import { useArtworkByTitle } from '../../hooks/useArtworkByTitle';

const AnnunciationExpo: React.FC = () => {
  const title = "annunciation";

  // Utilize o hook para buscar as obras relacionadas ao título
  const { data, loading, error } = useArtworkByTitle(title);

  return (
    <div className="container mx-auto py-10">
      <h1 className="text-4xl font-bold mb-8">Welcome to Dutch Artworks</h1>

      {loading && <p>Carregando obras...</p>}
      {error && <p className="text-red-500">{error}</p>}
      {!loading && !error && data.length === 0 && (
        <p>Nenhuma obra encontrada para {title}.</p>
      )}
